...
class Ex1Tests {
	...
	void shouldSucess() {
		...
		jobParametersBuilder.addString("fileName", new ClassPathResource("ex1.csv").getFile().getAbsolutePath());
		...
		assertThat(jdbcTemplate.queryForObject("SELECT count(*) FROM student", Integer.class)).isEqualTo(1);
	}
}
